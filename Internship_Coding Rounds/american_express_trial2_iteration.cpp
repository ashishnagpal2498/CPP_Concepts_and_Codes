...
vector<int> count_coins(vector<int> coinDenominations, int monetaryValue) {
  ...
  // bool ans = Soln_coins_pick(coinDenominations,monetaryValue,0,coins_selected);
    ...
    for( i=0;i<coinDenominations.size()||money<=0;i++)
    {
        for(int j=0;j<coinDenominations.size();j++)
        {
            if(coinDenominations[j]<=money)
            {   
                //coin is less than money , therefore we can pick that coin
                //picking that coin
                Soln_coins_pick.push_back(coinDenominations[j]);
                money = money - coinDenominations[j]; 
                break;
            }

            //Agar coin pick nhi hua 
        }
    }
    //now printing the soln ;
    if(money==0)
    {
        for(int i=0;i<Soln_coins_pick.size();i++)
        {
            cout<<Soln_coins_pick[i]<<"  ";
        }
    }
    else
    {
